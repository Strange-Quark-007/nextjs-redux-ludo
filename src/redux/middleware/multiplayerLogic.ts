...
import {
  ...
  updateCapture,
  addTokenToBoardList,
  removeTokenFromBoardList,
  endGame,
  setMultiplayer,
} from '../slices/game/index';
...
export const inPlayUserDisconnect = (userList: string) => (dispatch: AppDispatch, getState: () => RootState) => {
  ...
  if (countDisc == 3) dispatch(endGame());
...
export const validateTurn = (socket: Socket) => (dispatch: AppDispatch, getState: () => RootState) => {
  ...
  let shouldPassTurn =
    diceRoll != 6 &&
    player?.tokens.every((token) => token.locationStatus === 'spawn' || token.locationStatus === 'home');
  if (shouldPassTurn) {
    setTimeout(() => {
...
export const calculateAndMove =
  (socket: Socket, moveInput: MoveInput) => (dispatch: AppDispatch, getState: () => RootState) => {
    const state = getState();
    let diceRoll = state.gameState.diceRoll;
    ...
    if (token?.locationStatus === 'onLane') {
      ...
      if (newPos > laneData[playerKey].endSquareId) throw new Error('Cannot move this token');
    ...
    let isACapture =
      !squareMap[newSquareId]?.isSafeSquare &&
      filteredList.length == 1 &&
      filteredList[0].id.substring(0, 2) !== token.id.substring(0, 2);
    ...
    let existingTokens = filteredList.map((t) => state.players[+t.id.split('')[1] - 1].tokens[+t.id.slice(2) - 1]);
