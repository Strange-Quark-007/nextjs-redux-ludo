...
export const gameSlice = createSlice({
  ...
  reducers: {
    ...
    setDiceRoll: (state, action) => {
      ...
      if (action.payload == 6) state.isSix = true;
    },
    passTurn: (state, action: PayloadAction<string | undefined>) => {
      ...
      if (action.payload) {
        state.currentTurn = action.payload;
      } else state.currentTurn = calculateNextTurn(state);
    },
    updateMoveStatus: (state, action: PayloadAction<string | undefined>) => {
      state.diceRoll = 0;
      state.hasRolled = false;
      if (state.isSix) state.isSix = false;
      else if (state.isCapture) state.isCapture = false;
      else state.currentTurn = action.payload ? action.payload! : calculateNextTurn(state);
    },
    updateCapture: (state) => {
